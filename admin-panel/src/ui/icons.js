// Inline SVG icons (no external assets — CSP-friendly). Each returns an
// <svg> element sized 1em and inheriting currentColor.
function svg(paths, { size = 20, stroke = true } = {}) {
  const ns = 'http://www.w3.org/2000/svg';
  const el = document.createElementNS(ns, 'svg');
  el.setAttribute('viewBox', '0 0 24 24');
  el.setAttribute('width', size);
  el.setAttribute('height', size);
  el.setAttribute('fill', stroke ? 'none' : 'currentColor');
...
export function icon(name, opts) {
  const fn = icons[name] || icons.grid;
  return fn(opts);
}
