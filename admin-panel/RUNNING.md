...
_Planned for Phase 5._ The panel is static, so the production image is
nginx serving `index.html` + `src/**` on port **3005**, with an entrypoint that
templates `env.js` from `API_BASE`. A compose entry will **extend** the existing
`backend/docker-compose.yml` (using the backend's Docker service name for the
API base) without modifying it. See IMPLEMENTATION_PLAN.md → Phase 5.
